...
class Db {
  ...
  getImage (id, callback) {
    ...
    let task = co.wrap(function * () {
      ...
      if (!image) {
        // se defino en el expectationn 'not found' como error
  ...
  getImages (callback) {
    ...
    let task = co.wrap(function * () {
      ...
      // IMAGES devuelve un cursor, es un objeto en el cual se muede navegar ya sea even emiters  o el metodo NEXt para que entregue el siguiente resultado y tiene un metodo toArray
  ...
  getImagesByUser (userId, callback) {
    ...
    let task = co.wrap(function * () {
      ...
      let images = yield r.db(db).table('images').getAll(userId, { index: 'userId'}).orderBy(r.desc('createdAt')).run(conn)
      let result = yield images.toArray()
      return Promise.resolve(result)
    })
    return Promise.resolve(task()).asCallback(callback)
